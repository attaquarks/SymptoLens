...
export class ReasoningEngine {
  ...
  private async generatePredictions(fusionOutput: MultimodalFusionOutput): Promise<PotentialCondition[]> {
    ...
    // Get all possible conditions from the knowledge base
    const allConditions = await knowledgeBase.getAllConditions();
    
    // For each condition, calculate a score based on symptom matching
    for (const condition of allConditions) {
      // Calculate a base score using the knowledge base's symptom association
      ...
      if (associationScore > 0.7) {
        relevance = 'high';
      } else if (associationScore > 0.4) {
        relevance = 'medium';
      }
      ...
      if (matchingFactors.length > 0 || associationScore > 0.3) {
        predictions.push({
          name: condition.name,
          ...
          reasoningNotes: [
            `Association score: ${associationScore.toFixed(2)}`,
